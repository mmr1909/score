...
import header_service.header_service as hs
import pytest
...
def test_setup():
    mock_app = MagicMock()
    hs.setup(mock_app)
    mock_app.connect.assert_called_once_with("env-before-read-docs", ANY)
...
def test_register():
    ...
    with patch("header_service.header_service.SphinxNeedsData", return_value=mock_data):
...
@patch("header_service.header_service.generate_hash")
@patch("header_service.header_service._extract_github_data")
...
@patch("header_service.header_service.generate_hash")
@patch("header_service.header_service._extract_merge_commit_data")
@patch("sphinx.application.Sphinx")
...
@patch("header_service.header_service.HeaderService.request_from_directive")
@patch("sphinx.application.Sphinx")
...
@patch("header_service.header_service.subprocess.run")
...
@patch("header_service.header_service._extract_approvers")
@patch("header_service.header_service._extract_reviewers")
@patch("header_service.header_service._extract_team_info")
@patch("header_service.header_service._extract_org")
@patch("header_service.header_service._extract_repo")
@patch("header_service.header_service._extract_pull_request")
@patch("header_service.header_service._extract_github_token")
@patch("header_service.header_service.Auth.Token")
@patch("header_service.header_service.Github")
...
def test_extract_team_info():
    ...
    assert hs._extract_team_info(mock_org) == {"WG Process": ["approver1", "approver3"]}


@patch("header_service.header_service._append_approver_teams")
